...
public final class TransformerBlockChest implements IASMClassTransformer
{
    ...
    public void transform(@Nonnull final ClassNode classNode) {
         ...
         * {
         *     Hooks.neighborChanged(this, state, worldIn, pos, fromPos);
         * }
         */
    ...
    public static final class Hooks
    {
        ...
        public static IBlockState checkForSurroundingChests(@Nonnull final World world, @Nonnull final BlockPos pos, @Nonnull final IBlockState state) {
            ...
            @Nullable final IBlockState replacement = sidelessCheckForSurroundingChests(new BlockSource(world, pos, state));
            if(replacement != null) {
        ...
        public static IBlockState getStateForPlacement(@Nonnull final BlockChest block, @Nonnull final World world, @Nonnull final BlockPos pos, @Nonnull final EnumFacing side, @Nonnull final EntityLivingBase placer) {
            @Nonnull final Pair<EnumFacing, ChestType> properties = getPropertiesForPlacement(placer, side, facing -> {
                ...
                return ((IChestMatchable)block).chestMatches(chest, other) && !ChestType.get(other.getBlockState()).hasSideAttached() ? other.getBlockState().getValue(BlockChest.FACING) : null;
            });

            return block.getDefaultState().withProperty(BlockChest.FACING, properties.getLeft()).withProperty(ChestType.TYPE, properties.getRight());
        }

        //helper
        @Nonnull
        public static Pair<EnumFacing, ChestType> getPropertiesForPlacement(@Nonnull final EntityLivingBase placer, @Nonnull final EnumFacing side, @Nonnull final UnaryOperator<EnumFacing> directionToAttach) {
            ...
            if(side.getAxis().isHorizontal() && isSneaking) {
                @Nullable final EnumFacing sideToAttach = directionToAttach.apply(side.getOpposite());
            ...
            if(type == ChestType.SINGLE && !isSneaking) {
                @Nullable final EnumFacing left = directionToAttach.apply(facing.rotateY());
                if(left != null && facing != left.getOpposite()) type = ChestType.LEFT;
                else {
                    @Nullable final EnumFacing right = directionToAttach.apply(facing.rotateYCCW());
                    if(right != null && facing != right.getOpposite()) type = ChestType.RIGHT;
                }
            }

            return Pair.of(facing, type);
        }
        ...
        public static void neighborChanged(@Nonnull final BlockChest block, @Nonnull final IBlockState state, @Nonnull final World world, @Nonnull final BlockPos pos, @Nonnull final BlockPos otherPos) {
            @Nullable final TileEntity tile = world.getTileEntity(pos);
            if(tile instanceof TileEntityChest) tile.updateContainingBlockInfo();

            @Nonnull final ChestType type = ChestType.get(state);
            if(type.hasSideAttached() && pos.offset(type.getSideAttached(state)).equals(otherPos)) {
        ...
        // helper
        @Nullable
        public static IBlockState sidelessCheckForSurroundingChests(@Nonnull final IBlockSource chest) {
            @Nonnull final World world = chest.getWorld();
            @Nonnull final BlockPos pos = chest.getBlockPos();
            @Nonnull final IBlockState state = chest.getBlockState();
            @Nonnull final ChestType type = ChestType.get(state);
            switch(type) {
                case LEFT:
                case RIGHT: {
